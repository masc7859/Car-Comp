...
int main(int argc, char **argv){
  ...
  std::fstream gyro_file("/home/maxrebo/Documents/robotics_class/lab_run_ninja_imu_data/gyro.txt", std::ios_base::in);
  ...
  std::fstream time_file("/home/maxrebo/Documents/robotics_class/lab_run_ninja_imu_data/timestamp.txt", std::ios_base::in);
  ...
  while ((time_file >> _sec_in) && time_file.ignore() && (time_file >> _nsec_in)){
      ...
      timestamp_in.__sec = _sec_in;
      timestamp_in.__nsec = _nsec_in;
  ...
  float roll_prev,pitch_prev,yaw_prev = 0.0;
  float roll_cur,pitch_cur,yaw_cur;
  float prev_time = timestamp_data.front()._sec;
  float cur_time = timestamp_data.front()._sec;
  float prev_time_real = timestamp_data.front().__sec;
  float cur_time_real = timestamp_data.front().__sec;
  float time_interval;
  ...
  while(n.ok()){
    ...
    cur_time_real = timestamp_data.front().__sec;
    time_interval = cur_time_real - prev_time_real;
    ...
    ROS_INFO("%s: %d %d %f", "Published Both at", static_cast<int>(cur_time), static_cast<int>(prev_time), cur_time_real);
    ...
    odom.pose.covariance[14] = 1000.0;
    odom.pose.covariance[21] = 1000.0;
    odom.pose.covariance[28] = 1000.0;
    odom.pose.covariance[35] = 1000.0;

    odom.twist.covariance[0] = 1000.0;
    odom.twist.covariance[7] = 1000.0;
    ...
    imu.linear_acceleration_covariance[0] = 1000.0;
    imu.linear_acceleration_covariance[4] = 1000.0;
    imu.linear_acceleration_covariance[8] = 1000.0;

    imu.angular_velocity_covariance[0] = 1000.0;
    imu.angular_velocity_covariance[4] = 1000.0;
    imu.angular_velocity_covariance[8] = 1000.0;
    ...
    prev_time_real = timestamp_data.front().__sec;
